    ...
    public List<TimelineEvent> simulate(List<ProcessSimulation> processes) {
        ...
        while (!remaining.isEmpty()) {
            ...
            if (!available.isEmpty()) {
                next = available.stream().min(Comparator.comparingInt(ProcessSimulation::getBurstTime)).get();
            } else {
                next = remaining.stream().min(Comparator.comparingInt(ProcessSimulation::getArrivalTime)).get();
